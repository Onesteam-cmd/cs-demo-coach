...
LAYER_VERSION = "canonical_info_state_v0_1"
...
def detect_common_cols(df: Optional[pd.DataFrame], kind: str) -> Dict[str, Optional[str]]:
    ...
    if kind == "grenades":
        return {
            "round": find_col(cols, ["round_num", "roundNumber", "round_number", "round"], ["round"]),
            "tick": find_col(cols, ["tick", "throw_tick", "game_tick", "gameTick"], ["tick"]),
            "player": find_col(cols, ["player_name", "playerName", "thrower_name", "throwerName", "thrower"], ["player", "thrower"]),
            "side": find_col(cols, ["player_side", "side", "team_side"], ["side"]),
            "grenade_type": find_col(cols, ["grenade_type", "grenadeType", "projectile_type", "type"], ["grenade", "type"]),
...
def build_player_side_index(dfs: Dict[str, Optional[pd.DataFrame]], player: str) -> Dict[Tuple[int, str], str]:
...
def infer_actor_side(round_num: int, actor: str, explicit_side: str, player_side_index: Dict[Tuple[int, str], str]) -> str:
...
def build(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    table_paths = {
        ...
        "grenades": parsed_dir / "grenades.parquet",
    ...
    if missing:
        warnings.append("missing optional parsed tables: " + ", ".join(missing))
    ...
    player_side_index = build_player_side_index(dfs, player)

    observations: List[Dict[str, Any]] = []
    focus_events: List[Dict[str, Any]] = []

    # Kills: high-confidence killfeed / direct contact info
    kills = dfs.get("kills")
    if kills is not None:
        c = detect_common_cols(kills, "kills")
        ...
        for i, row in kills.iterrows():
            ...
            if attacker_is_player and victim:
                add_observation(
                    observations, event_id, rn, tick, "player_kill_contact",
                    victim, victim_side or e_side, "player", p_side,
                    "player_killed_enemy", "high", "kills",
                    place, vx, vy, vz, f"weapon={weapon}"
                )
                focus_events.append({
                    "event_id": event_id,
                    "round_num": rn,
                    "tick": tick,
                    "focus_event_kind": "player_kill",
                    "opponent": victim,
                    "opponent_side": victim_side or e_side,
                    "weapon": weapon,
                })

            if victim_is_player and attacker:
                add_observation(
                    observations, event_id, rn, tick, "player_death_contact",
                    attacker, attacker_side or e_side, "player", p_side,
                    "enemy_killed_player", "high", "kills",
                    place, ax, ay, az, f"weapon={weapon}"
                )
                focus_events.append({
                    "event_id": event_id,
                    "round_num": rn,
                    "tick": tick,
                    "focus_event_kind": "player_death",
                    "opponent": attacker,
                    "opponent_side": attacker_side or e_side,
                    "weapon": weapon,
                })

            # Team-side killfeed info
            if p_side and e_side:
                if attacker and attacker_side == e_side and victim_side == p_side:
                    add_observation(
                        observations, event_id, rn, tick, "enemy_killed_teammate",
                        attacker, attacker_side, "player_team", p_side,
                        "killfeed_enemy_position_proxy", "medium", "kills",
                        place, ax, ay, az, f"victim={victim}; weapon={weapon}"
                    )

                if victim and victim_side == e_side and attacker_side == p_side:
                    add_observation(
                        observations, event_id, rn, tick, "teammate_killed_enemy",
                        victim, victim_side, "player_team", p_side,
                        "killfeed_enemy_death_position", "high", "kills",
                        place, vx, vy, vz, f"attacker={attacker}; weapon={weapon}"
                    )

    # Damages: medium-confidence contact info
    damages = dfs.get("damages")
    if damages is not None:
        c = detect_common_cols(damages, "damages")
        ...
        for i, row in damages.iterrows():
            ...
            if attacker_is_player and victim:
                add_observation(
                    observations, event_id, rn, tick, "player_damaged_enemy",
                    victim, victim_side or e_side, "player", p_side,
                    "player_damage_contact", "medium", "damages",
                    place, vx, vy, vz, f"damage={damage}"
                )
                focus_events.append({
                    "event_id": event_id,
                    "round_num": rn,
                    "tick": tick,
                    "focus_event_kind": "player_damage_dealt",
                    "opponent": victim,
                    "opponent_side": victim_side or e_side,
                    "damage": damage,
                })
            ...
            if victim_is_player and attacker:
                add_observation(
                    observations, event_id, rn, tick, "enemy_damaged_player",
                    attacker, attacker_side or e_side, "player", p_side,
                    "enemy_damage_contact", "medium", "damages",
                    place, ax, ay, az, f"damage={damage}"
                )
                focus_events.append({
                    "event_id": event_id,
                    "round_num": rn,
                    "tick": tick,
                    "focus_event_kind": "player_damage_taken",
                    "opponent": attacker,
                    "opponent_side": attacker_side or e_side,
                    "damage": damage,
                })
            ...
            if p_side and e_side:
                if attacker and attacker_side == e_side and victim_side == p_side:
                    add_observation(
                        observations, event_id, rn, tick, "enemy_damaged_teammate",
                        attacker, attacker_side, "player_team", p_side,
                        "damage_enemy_position_proxy", "medium", "damages",
                        place, ax, ay, az, f"victim={victim}; damage={damage}"
                    )

                if victim and victim_side == e_side and attacker_side == p_side:
                    add_observation(
                        observations, event_id, rn, tick, "teammate_damaged_enemy",
                        victim, victim_side, "player_team", p_side,
                        "damage_enemy_hit_position", "medium", "damages",
                        place, vx, vy, vz, f"attacker={attacker}; damage={damage}"
                    )

    # Grenades: lower-confidence presence / utility-pressure proxy
    grenades = dfs.get("grenades")
    if grenades is not None:
        c = detect_common_cols(grenades, "grenades")

        for i, row in grenades.iterrows():
            d = row.to_dict()
            rn = safe_int(get(d, c.get("round")))
            tick = safe_int(get(d, c.get("tick")))
            if rn is None or tick is None:
            ...
            add_observation(
                observations, f"grenade_{i}", rn, tick, "enemy_utility_presence_proxy",
                actor, actor_side, "player_team", p_side,
                "enemy_utility_thrown", "low", "grenades",
                place, x, y, z, f"grenade_type={grenade_type}"
            )

    # Bomb: objective info proxy
    bomb = dfs.get("bomb")
    if bomb is not None:
        c = detect_common_cols(bomb, "bomb")
        ...
        for i, row in bomb.iterrows():
            ...
            if "plant" in event or "defuse" in event or "bomb" in event:
                ...
                add_observation(
                    observations, f"bomb_{i}", rn, tick, "objective_info",
                    subject, side, "player_team", p_side,
                    "objective_event", confidence, "bomb",
                    place, x, y, z, f"event={event}; site={site}"
                )

    observations.sort(key=lambda x: (x["round_num"], x["tick"], x["event_id"]))
    focus_events.sort(key=lambda x: (x["round_num"], x["tick"], x["event_id"]))

    # Build per-round, per-enemy last-known snapshots.
    ...
    for focus in focus_events:
        ...
        for obs in obs_by_round.get(rn, []):
            ...
            if existing is None:
                ...
                # Prefer newer info. If same tick, prefer higher confidence.
                if int(obs["tick"]) > int(existing["tick"]):
                    last_by_subject[subject_key] = obs
                elif int(obs["tick"]) == int(existing["tick"]) and confidence_rank(obs["confidence"]) > confidence_rank(existing["confidence"]):
                    last_by_subject[subject_key] = obs
        ...
        if opponent_info:
            context = opponent_info["freshness"]
        else:
            context = "no_prior_info"
        ...
        focus_snapshots.append({
            ...
            "interpretation_v0_1": {
                ...
                "note": (
                    "по противнику была свежая/недавняя инфа"
                    if context in ("fresh", "recent")
                    else "по противнику не было актуальной инфы или она устарела"
                )
            }
        })
    ...
    summary = {
        ...
        "known_limitations_v0_1": [
            ...
            "utility observations are presence proxies, not guaranteed direct player vision",
            "tick rate is assumed as 64 unless later provided by parser metadata",
            "this layer estimates what could be known from parsed events, not actual team comms",
        ],
    }
    ...
    package = {
        "meta": {
            ...
            "match_id": match_id,
            "player": player,
            "purpose": "canonical player/team information-state reconstruction for coach reasoning",
        },
        "summary": summary,
    ...
    out_json = out_dir / f"canonical_info_state_{player}_v0_1.json"
    out_csv = out_dir / f"canonical_info_state_{player}_v0_1.csv"
    ...
    with out_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for s in focus_snapshots:
            ...
            interp = s.get("interpretation_v0_1") or {}
