...
from backend.package.match_package_builder_v0_3 import rows_for_csv
from backend.package.match_package_builder_v0_1 import build_package, file_entry


VERSION = "match_package_builder_v0_4"
...
def main() -> None:
    ...
    print("=== Match Package Builder v0.4 ===")
    ...
    combat_json = data_root / "analysis" / args.match_id / f"combat_profile_{args.player}_v0_1.json"
    combat_layer_json = data_root / "layers" / args.match_id / f"canonical_combat_events_{args.player}_v0_1.json"

    loss_patterns = load_json(loss_json)
    utility_value = load_json(utility_json)
    combat_profile = load_json(combat_json)
    combat_layer = load_json(combat_layer_json)

    package["summaries"]["loss_patterns"] = loss_patterns.get("summary", {})
    package["summaries"]["utility_value"] = utility_value.get("summary", {})
    package["summaries"]["combat_profile"] = combat_profile.get("summary", {})
    package["summaries"]["combat_layer"] = combat_layer.get("summary", {})
    ...
    package["coach"]["combat_profile"] = {
        "top_weapons": combat_profile.get("summary", {}).get("top_weapons", []),
        "top_combat_rounds": combat_profile.get("summary", {}).get("top_combat_rounds", []),
        "combat_label_counts": combat_profile.get("summary", {}).get("combat_label_counts", {}),
    }

    package["files"]["loss_patterns"] = file_entry(loss_json)
    package["files"]["utility_value"] = file_entry(utility_json)
    package["files"]["combat_profile"] = file_entry(combat_json)
    package["files"]["canonical_combat_events"] = file_entry(combat_layer_json)
    ...
    package["health"]["checks"]["combat_rounds_total"] = combat_profile.get("summary", {}).get("rounds_total")
    package["health"]["checks"]["combat_kd"] = combat_layer.get("summary", {}).get("player_kd")

    out_dir = data_root / "package" / args.match_id
    json_path = out_dir / f"match_package_{args.player}_v0_4.json"
    csv_path = out_dir / f"match_package_index_{args.player}_v0_4.csv"

    write_json(json_path, package)
    write_csv(csv_path, rows_for_csv(package))

    print("")
    print("=== MATCH PACKAGE v0.4 COMPLETE ===")
    print(f"JSON: {json_path}")
    print(f"CSV:  {csv_path}")
    print("")
    print_json({
        "health": package.get("health"),
        "combat_profile": package.get("summaries", {}).get("combat_profile", {}),
