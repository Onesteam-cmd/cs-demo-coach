...
from backend.package.match_package_builder_v0_1 import build_package, rows_for_csv, file_entry


VERSION = "match_package_builder_v0_2"
...
def main() -> None:
    ...
    print("=== Match Package Builder v0.2 ===")
    ...
    if package.get("health", {}).get("status") == "ok":
        package["health"]["status"] = "ok"
    ...
    json_path = out_dir / f"match_package_{args.player}_v0_2.json"
    csv_path = out_dir / f"match_package_index_{args.player}_v0_2.csv"

    write_json(json_path, package)
    write_csv(csv_path, rows_for_csv(package))

    print("")
    print("=== MATCH PACKAGE v0.2 COMPLETE ===")
