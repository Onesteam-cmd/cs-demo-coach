...
VERSION = "match_package_builder_v0_6"
...
def optional_load(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {}
    return json.loads(path.read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    print("=== Match Package Builder v0.6 ===")
    ...
    addon_paths = {
        ...
        "phase_timeline": data_root / "layers" / args.match_id / f"canonical_phase_timeline_{args.player}_v0_1.json",
        "phase_profile": data_root / "analysis" / args.match_id / f"phase_profile_{args.player}_v0_1.json",
        "round_state": data_root / "layers" / args.match_id / f"canonical_round_state_{args.player}_v0_1.json",
    ...
    package["health"]["checks"]["loss_rounds_total"] = addons["loss_patterns"].get("summary", {}).get("loss_rounds_total")
    package["health"]["checks"]["utility_problem_rounds_total"] = addons["utility_value"].get("summary", {}).get("problem_rounds_total")
    package["health"]["checks"]["combat_rounds_total"] = addons["combat_profile"].get("summary", {}).get("rounds_total")
    package["health"]["checks"]["combat_kd"] = addons["combat_layer"].get("summary", {}).get("player_kd")
    package["health"]["checks"]["phase_problem_rounds_total"] = addons["phase_profile"].get("summary", {}).get("problem_rounds_total")
    package["health"]["checks"]["main_problem_phase"] = addons["phase_profile"].get("summary", {}).get("main_problem_phase")
    package["health"]["checks"]["player_swing_events_total"] = addons["advantage_profile"].get("summary", {}).get("player_swing_events_total")

    out_dir = data_root / "package" / args.match_id
    json_path = out_dir / f"match_package_{args.player}_v0_6.json"
    csv_path = out_dir / f"match_package_index_{args.player}_v0_6.csv"

    write_json(json_path, package)
    write_csv(csv_path, rows_for_csv(package))

    print("")
    print("=== MATCH PACKAGE v0.6 COMPLETE ===")
    print(f"JSON: {json_path}")
    print(f"CSV:  {csv_path}")
    print("")
    print_json({
        "health": package.get("health"),
        "advantage_profile": package.get("summaries", {}).get("advantage_profile", {}),
