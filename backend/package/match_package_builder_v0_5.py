...
from backend.package.match_package_builder_v0_4 import rows_for_csv
from backend.package.match_package_builder_v0_1 import build_package, file_entry


VERSION = "match_package_builder_v0_5"
...
def main() -> None:
    ...
    print("=== Match Package Builder v0.5 ===")
    ...
    json_path = out_dir / f"match_package_{args.player}_v0_5.json"
    csv_path = out_dir / f"match_package_index_{args.player}_v0_5.csv"

    write_json(json_path, package)
    write_csv(csv_path, rows_for_csv(package))

    print("")
    print("=== MATCH PACKAGE v0.5 COMPLETE ===")
    print(f"JSON: {json_path}")
    print(f"CSV:  {csv_path}")
    print("")
    print_json({
        "health": package.get("health"),
        "phase_profile": package.get("summaries", {}).get("phase_profile", {}),
