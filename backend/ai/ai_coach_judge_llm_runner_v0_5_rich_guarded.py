...
def build_rich_guarded_input(match_id: str, player: str) -> dict[str, Any]:
    ...
    rich = {
        "version": "ai_coach_judge_input_v0_5_rich_guarded",
        "match_id": match_id,
        "player": player,
...
Твоя задача: дать качественный тренерский разбор игрока по evidence. Нужен не сухой пересказ, а полезный coach report. Но каждое утверждение должно быть grounded.
...
Схема ответа:
{
  "schema_version": "ai_coach_judge_report_v0_5_rich_guarded",
  "match_summary": "...",
  "quality_control": {
...
def main() -> None:
    ...
    rich_path = Path(f"data/ai/{match_id}/ai_coach_judge_input_rich_guarded_{player}_v0_5.json")
    rich_current = Path(f"data/ai/{match_id}/ai_coach_judge_input_rich_guarded_current.json")
    preview_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_request_preview_{player}_v0_5.json")
    prompt_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_prompt_preview_{player}_v0_5.txt")
    ...
    if args.mode == "check":
        print(json.dumps({
            ...
            "runner": "ai_coach_judge_llm_runner_v0_5_rich_guarded",
            "request_preview": preview,
            "created": {
                "rich_input": str(rich_path),
                "rich_current": str(rich_current),
    ...
    result_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_result_{player}_v0_5.json")
    report_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_report_{player}_v0_5.json")
    report_txt_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_report_{player}_v0_5.txt")
    status_path = Path(f"data/ai/{match_id}/ai_coach_judge_llm_status_{player}_v0_5.json")
